...
import ballerinax/mssql;                    //remove
import ballerina/io;                        //remove
import ballerinax/mssql.driver as _;        //remove
...
isolated client class SchemaClient {
    private final mssql:Client dbClient;                                                                                    //remove mssql
    ...
    public function init(string host, string user, string password, string database,
        int port, string instance = "", mssql:Options? options = (), sql:ConnectionPool? connectionPool = ()) returns sql:Error? {          //remove mssql
        self.database =  database;
        self.dbClient = check new (HOST, USER, PASSWORD, DATABASE, PORT);                                                                   //add old vars back: self.dbClient = check new (host, user, password, database, port, instance, options, connectionPool);
    }
